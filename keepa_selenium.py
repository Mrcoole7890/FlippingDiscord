...
# get google.co.in
driver.get(os.getenv("keepaMainPage"))
...
for i in range(0, 105):
    ...
    if (int(percentOff[0:2]) >= 90):
        sendPercentageOffMessageToDiscord("amazon90percentoff", avgPrice, currPrice, img, percentOff, title, asin)

    elif (int(percentOff[0:2]) >= 80):
        sendPercentageOffMessageToDiscord("amazon80percentoff", avgPrice, currPrice, img, percentOff, title, asin)

    elif (int(percentOff[0:2]) >= 70):
        sendPercentageOffMessageToDiscord("amazon70percentoff", avgPrice, currPrice, img, percentOff, title, asin)

    elif (int(percentOff[0:2]) >= 50):
        sendPercentageOffMessageToDiscord("amazon50percentoff", avgPrice, currPrice, img, percentOff, title, asin)